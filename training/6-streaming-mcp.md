...
**src/mcp-server.ts** (add the countdown tool):
```typescript
...
export function createMcpServer(): McpServer {
  ...
  // Register the slugify tool from previous steps
  server.registerTool(
    "slugify",
    {
      title: "Slugify",
  ...
  server.registerTool(
    ...
    async ({ seconds }, extra) => {
      ...
      for (let i = seconds; i > 0; i--) {
        ...
        // 2. Progress notification (for general clients)
        await extra.sendNotification({
          method: "notifications/message",
          params: {
            level: "info",
            data: `Progress: ${seconds - i + 1}/${seconds} - ${i} seconds left`
          }
        });
